...
## Where this repository's delta/ lives, or will
...
2. Otherwise check the current directory, then each parent in turn, for a
   `delta/` directory. If found, that is the root, and it already has a
   `delta/` — skip straight to "Read first" below.
3. If none is found, do the same walk again looking for `.git` instead. That
   is the root. This repository has never run `propose`, so `delta/` does not
   exist yet — go to "Creating delta/ for the first time" below before
   anything else.
...
## Creating delta/ for the first time

Skip this section entirely if `delta/` already exists at the resolved root.

Adding delta to a repository is nothing more than this happening once, the
first time someone runs `propose` in it. There is no separate init command,
and there will not be one — see the constitution template's own note on why,
right below.

1. `mkdir -p delta/truth delta/changes delta/bin`

2. Write `delta/constitution.md` with **exactly** the template below —
   verbatim, character for character. Do not fill in the blanks, do not
   invent example rules, and do not introspect the repository to write real
   ones — that is the one thing this lifecycle refuses to automate, on
   purpose, because auto-generated context files measurably reduce agent
   success. The next person edits this file by hand; your job is only to put
   the template where they will find it.
...
`changes/` start empty — which "Greenfield is not a special mode" below
already covers, since an empty `truth/` is not a special case either.
...
As soon as `<id>` is known, run this literally and print its output verbatim,
then a blank line — before writing anything:

```sh
delta/bin/stage-rail propose <id>
```

If `delta/bin/stage-rail` does not exist, or there is no `delta/` yet at this
point in the lazy-init flow above, skip this section silently.
...
1. `delta/constitution.md` — inherited by every change. Its rules constrain
   ...
   judgement call. Immediately after creating it from the template, it has
   nothing but prompts in it, which constrains nothing yet — that is expected
